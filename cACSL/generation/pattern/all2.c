 ...
 * the form
 *  \forall integer i; 0 <= i < n ==> (\forall integer j; 0 <= j < n ==> phi),
 ...
 *  frama-c -wp all2.c -wp-skip-fct b_x3 */
...
int b_x2(int a[], int n, int v1) {
 ...
 /*@ loop invariant 0 <= v1 <= i <= n;
   @ loop invariant is_x2_gen(a,n,v1,i);
   @ loop assigns i;
   @ loop variant n-i; */
 for (i = v1; i < n; i++)
  if (b_x3(a,n,v1,i) == 0) return 0;
 return 1;
}
