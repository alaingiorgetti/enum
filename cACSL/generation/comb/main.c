...
int main(int argc,char *argv[]) {
 int i, p, n;
 long d;
 int c[MAX]; // array of combination
 ...
 if (p > MAX) {
  printf("Invalid first argument: The minimal number should be structurally %d or less.\n",MAX);
  return(-1);
 }
 ...
 if (p > n) {
  printf("Invalid second argument: Should be greater or equal to the first argument.\n");
  return(-1);
 }
